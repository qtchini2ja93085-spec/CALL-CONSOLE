...
(function(){
  ...
  function add(){
    ...
    host.innerHTML='<div class="aero-title">SCRIPT INTELLIGENCE</div><div class="aero-sub">Choose how CALLCONSOLE tailors the script. Each option runs immediately. Original/local is never overwritten.</div><div class="aero-btns"><button type="button" data-mode="auto">⚡ AUTO / SMART</button><button type="button" data-mode="ai">🧠 AI TAILOR</button><button type="button" data-mode="local">📄 ORIGINAL / LOCAL</button></div><div id="aero-script-status">⚡ Auto mode ready.</div><div id="aero-ai-script"><div class="aero-ai-label">🧠 AI TAILORED SCRIPT</div><div id="aero-ai-script-text" class="aero-ai-script-text"></div><div id="aero-ai-meta" class="aero-ai-meta"></div><div id="aero-ai-qs" class="aero-ai-q"></div></div>';
  ...
  function providerLabel(source){const labels={gemini:'🧠 GEMINI',groq:'⚡ GROQ',cerebras:'🚀 CEREBRAS',local:'📄 LOCAL AERO'};return labels[source]||'🧠 AI PROVIDER';}
  ...
  async function run(mode){const generated=ensureLocalScript();const c=context();setMode(mode);if(mode==='local'){status(generated?'📄 ORIGINAL / LOCAL ACTIVE — script generated and ready.':'📄 ORIGINAL / LOCAL ACTIVE — proven deterministic tailoring.');clearAI();return;}if(mode==='auto'){const d=decision(c);if(d.ai){status('⚡ AUTO → 🧠 AI TAILOR — '+d.reason);return run('ai');}status('⚡ AUTO → 📄 ORIGINAL / LOCAL — '+d.reason);clearAI();return;}status('🧠 AI TAILORING…');const out=$('#aero-ai-script');if(out)out.classList.add('open');try{const fresh=context();const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...fresh,task:'script_tailoring'})});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();if(!d||!d.script)throw new Error('No tailored script returned');const text=$('#aero-ai-script-text');if(text)text.textContent=d.script;const meta=$('#aero-ai-meta');if(meta)meta.textContent=providerLabel(d.source)+' • Confidence: '+(d.confidence||'unknown')+' • '+(d.reason||'Evidence-based tailoring');renderQuestions(d.discoveryQuestions);status(d.source==='local'?'📄 LOCAL AERO — external providers unavailable; original/local preserved.':providerLabel(d.source)+' TAILORING APPLIED — separate AI version generated.');}catch(e){clearAI();status('⚠️ AI tailoring failed — ORIGINAL / LOCAL remains available.');}}
